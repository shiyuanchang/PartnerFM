...
| CLI | `.partnerfm-cli.json` | CLI 工具注册表（只读文档看板） |
| MCP | `.partnerfm-mcp.json` | MCP 服务器注册表（只读文档看板） |
| 提示词 | `.partnerfm-prompts.json` | 系统提示词 CRUD，与聊天下拉框打通 |
...
### PPT 转 PDF 预览（服务端代码已写好，等待 LibreOffice 安装）
- **状态**：server.py 的 `/api/convert-office` 端点已完成，前端 PPT 打开时先尝试服务端转换，失败则降级为 JSZip 文字提取
- **阻塞**：需要安装 LibreOffice：下载 https://www.libreoffice.org/download/ 的 macOS DMG，拖入 `/Applications`
- **安装后验证**：`ls /Applications/LibreOffice.app/Contents/MacOS/soffice` 存在即可
- **相关文件**：`server.py:_convert_office()`、`index.html:loadFileContent` pptx 分支

## 遗留问题

### 聊天区横条与左侧模块区未对齐（未解决）
- **现象**：聊天区顶部横条（tabs 行、模型/提示选择行）与左侧模块内容区（mgmt-panel）存在肉眼可见的水平错位
- **尝试过的修复**：统一聊天区各横条左右内边距为 16px，但未解决与左侧模块区的对齐问题
- **根因**：可能涉及聊天区外层容器（`#chat-column`）的 border/padding 与中心面板（`#center-panel`）的差异，需进一步排查
