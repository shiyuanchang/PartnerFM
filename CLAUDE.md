...
### 文件夹/文件拖拽排序和移动不工作（未解决）
- **现象**：拖拽文件树中的文件或文件夹，不会出现蓝色插入线或高亮，无法排序也无法移入子文件夹
- **尝试过的修复**：
  - 侧边栏 `dragenter`/`dragover`/`drop` 加 `isExternalDrag()` 判断（只处理 Finder 拖入，不拦截内部拖拽）
  - `dragstart` 加 `e.dataTransfer.setData('text/plain', ...)`（Chrome 要求）
  - 文件夹头部加 `draggable=true` 和排序/移入双模式 `dragover`/`drop` 处理
  - 树插入改 `insertIntoTreeNodes()` 做 name 兜底
- **均无效**，根因未确定
- **临时替代**：暂无。排序可手动编辑 localStorage key `partnerfm-folder-order`，移动文件需在访达中操作
